...
for i in data_file:
    if i.startswith("#"):
        ...
        fields = i.rstrip("\r\n").split("\t")
        quals = "".join(fields[5])
        quality_distribution.append(float(quals))
...
for i in data_file:
    if i.startswith("#"):
        ...
        info = fields[7]
        allele = info.split(";")[3]
        allele_q = allele.split("=")[1]
            
        if "," in allele:
           allele_spl = allele_q.split(",")
           allele_freq.append(float(allele_spl[0]))
           allele_freq.append(float(allele_spl[1]))
        else:
           allele_freq.append(float(allele_q))
#print(allele_freq)              
